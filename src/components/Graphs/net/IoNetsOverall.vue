...
export default {
	...
	methods: {
		...
		fastAddNewData: function (elem) {
			...
			const { recv, sent } = this.getReadWriteFrom(currDate, total_recv, total_sent)

			// Add the new value to the Array
			this.pushValue(currDate, recv, sent, total_recv, total_sent)
		},
		addNewData: function () {
			const vm = this
			let total_recv = 0
			...
			const currDate = moment.utc(vm.bufferDataWs[0][11]).unix();
