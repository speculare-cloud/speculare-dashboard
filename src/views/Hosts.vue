<template>
	<div class="hosts">
		...
		<div v-if="this.$store.state.category.length" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:xl:grid-cols-5 gap-4 mb-8">
			<div v-for="item in this.$store.state.category" v-bind:key="item.id" class="flex items-center text-gray-200 bg-gray-800 relative shadow px-4 py-3 rounded-lg hover:shadow-lg hover:bg-gray-600 hover:text-gray-100">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 2xl:xl:grid-cols-5 gap-4 mb-8">
			...
			<div v-if="!this.$store.state.hosts_values.length" class="shimming flex items-center overflow-hidden bg-gray-800 relative shadow py-3 rounded-lg">
